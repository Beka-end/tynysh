      ...
      >
        ...
        <p className="text-xs text-tynysh-muted">
          {isPhone
            ...
            : "Пришлём письмо с кодом. Подойдёт любая почта — школьная, Gmail, Mail. Её никто не увидит: другие видят только твой @юзернейм."}
        </p>
      </form>
    );
  }
    ...
    >
      <p className="text-sm text-tynysh-muted">
        {isPhone ? (
          ...
          <>
            Письмо отправлено на {normalizeEmail(contact)}. Введи код из письма
            или просто нажми в нём на ссылку — подойдёт любой способ. Проверь и
            папку «Спам». Открывай <b>самое последнее</b> письмо: как только придёт
            новое, предыдущее перестаёт работать.
          </>
        )}
      </p>
      <input
      ...
      {!isPhone && (
        <p className="text-xs text-tynysh-muted">
          Код надёжнее ссылки: его не «съедает» почтовый робот и он работает, даже
          если письмо открыто на другом устройстве.
        </p>
      )}
    </form>
  );
